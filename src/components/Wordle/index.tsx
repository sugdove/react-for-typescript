...
export default function App() {
  ...
  const RANDOM_PWD = WORDS[parseInt(String((WORDS.length - 1) * Math.random()))]
  ...
  // 单击键盘 key为对应字母
  function click(obj: { [i: string]: any }) {
  ...
  // 字符串匹配
  function matchWord(word: string, rightWord: string) {
  ...
  function changeKeyBoardsStatus(arr: Array<any>) {
  ...
  // 单击字母或Delete
  function addOrDel(type: string, copyArr: Array<any>, key: string) {
    const copyStep = deepClone(step)
  ...
  function getBorderColor(obj: { [i: string]: any }): any {
